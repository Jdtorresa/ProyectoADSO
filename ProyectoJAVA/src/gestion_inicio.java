...
public class gestion_inicio {
    ...
    public inicio ConsultarInicio(String idinicio){
        inicio inic=null;
        try{
            conec=con.conecta();
            String sql="select * from inicio where idinicio=?";
            ps=conec.prepareStatement(sql);
            ps.setString(1, idinicio);
            res=ps.executeQuery();
            while(res.next()){
                inic=new inicio(res.getString(1), res.getString(2), res.getString(3), res.getString(4));
            }
    ...
    public ArrayList<inicio> consultarIniciosPorUsuario(String iduser){
        ...
        try {
            ...
            res=ps.executeQuery();
            while (res.next()){
                inicio inic=new inicio(res.getString(1), res.getString(2), res.getString(3), res.getString(4));
                datos.add(inic);
            }
        }catch (SQLException ex){
            System.out.println("Error al consultar: "+ex);
    ...
    public boolean eliminarInicio(String idInicio){
    ...
    public boolean insertarInicio(inicio inic) {
        ...
        try {
            ...
            ps.setString(3, inic.getIdinicio());
    ...
    public boolean editarInicio(inicio inicio) {
        ...
        try {
            ...
            ps.setString(4, inicio.getIdinicio());
            resultado = ps.executeUpdate() > 0;
        } catch (SQLException ex) {
